...
class SponsoMetabox {
    ...
    const META_KEY = 'gimmickshelter_sponso';
    const NONCE = '_gimmicksheter_sponso_nonce';
    ...
    public static function add ($postType, $post) {
        if ($postType === 'post' && current_user_can( 'publish_posts', $post)) {
            add_meta_box(self::META_KEY, 'Lien sponsorisé', [self::class, 'render'],'post', 'side');
        }
    }
    ...
    public static function render ($post) {
        ...
        <label for="gimmicksheltersponso">Cet article est il sponsorisé ?</label>
        <?php
    }
